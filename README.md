...
Resample the named rasters to a coarser cell size (for example 2 m to 5 m), in batch. An optional utility. Outputs go to a `Resample` folder inside the folder you point at, grouped by area, with the file names unchanged, so the other tools read them the same way. **Point it at the whole project root** and it finds every product wherever it was written (the surfaces and reclass in the mosaics tree, the suitability masks in their own folder), and consolidates all the 5 m outputs into that one `Resample` folder, so the delivery tools then read a single place.

| Parameter | Description |
| --- | --- |
| Results folder | The folder to scan; a `Resample` subfolder is created inside it. Can be a single results tree or the whole project root. |
| Recurse subfolders | On (default). |
| Data types to resample | Multi-select. The list is filled from what actually exists under the folder you pick, so you only see products that are there. Default `DEM`, `DSM` when nothing is detected yet. |
...
| Source resolution when a product exists in several | `finest` (default, matches the Suitability Mask) or `coarsest`. Used only when the same product is present in more than one resolution tree (for example 2 m and 50 cm); one is chosen so the outputs do not collide and the run over a whole project consolidates cleanly. |
| Overwrite existing outputs | Off skips existing resampled rasters. |

In `auto` the method is chosen per type: bilinear for continuous rasters, nearest for the class rasters (`_RCL`, `APT`, `APTCLS`, `ASPECT_DIR`) so the ordinal classes are preserved, and nearest for `ASPECT` too, which is continuous but circular (averaging 355 and 5 degrees gives 180, turning north into south) and carries -1 for flat as a sentinel that must not be averaged. **To generalize a class raster, force `MAJORITY`**: nearest only subsamples and keeps the speckle, while majority gives each output cell the most common class in the window, which is what actually simplifies the data (going from 0.5 m to 5 m, each output cell summarizes 100 input cells). Forcing an interpolating method (`BILINEAR`, `CUBIC`) on a class raster is allowed but warned about at the end of the run, since it invents class values that are not in the legend. A raster already at the target cell size is copied through unchanged, so the `Resample` folder stays a complete set; a target finer than the native cell size warns, since upsampling adds no real detail. A selected type with no raster under the folder is reported at the end (no silent skips), so a run that found nothing for `APT` says so. After a class raster is resampled, any cell whose value is not one of the source's classes is set to NoData and the count is reported: ArcGIS `Resample` with `MAJORITY` can read an 8-bit mask's NoData region and stamp out-of-range values (for example 4, 27, 207) along the boundary, and this scrub removes them so the delivery never carries a class that does not exist. Because all the 5 m outputs land in one `Resample` folder, the delivery tools (Merge Class Rasters, Vectorize Class Rasters) then read that single folder instead of hunting the separate trees.
...
Each mine is matched to the area raster that covers it, by name first (an area named like the mine) and spatially otherwise (the raster whose extent contains the most polygon centroids), so it works on per area results and on cluster results; a warning is issued when part of a mine falls outside the chosen raster. When the results folder holds more than one resolution tree (for example the `2m` and `50cm` trees Tool 2 builds), it uses the finest resolution present for both the slope and the solar of each area, the same one for both so a mine never mixes grids, and reports a single summary line. The computation is bounded to the mine polygons (snapped to the slope grid) and clipped to their shape with Extract By Mask. Cells that are NoData in either input stay NoData in the mask (only cells with data become 1 or 0). Both `APT` and `APTCLS` are written as 8 bit unsigned with a **fixed NoData of 255**. Left to choose, ArcGIS derives the pixel type and NoData from the content, and on a small or degenerate mine (for example one that is entirely suitable, so the only value present is `1`) it can pick a NoData equal to a real class (`0` or `1`), which makes that class read as empty and, worse, differ between mines so a later merge cannot tell data from NoData. 255 is never a class of `APT` `{0,1}` or `APTCLS` `{0,3,4,5,6}` and is the same for every mine, so the masks stay correct on their own and merge cleanly.
...
**Why sparse matters.** Areas of interest are usually scattered (in this project, mines from Valença to Faro), so the merged grid has to span their whole bounding box while only a tiny fraction carries data. At 5 m that is billions of cells and >99.9% NoData, past the 4 GB limit of a classic TIFF. The tool writes it with GDAL `Warp`, which snaps every area onto one common grid and outputs a **sparse, LZW compressed, tiled BigTIFF** (`SPARSE_OK=TRUE`, `BIGTIFF=YES`): the all-NoData blocks are never written, so the file stays small and the run stays quick. Nearest resampling keeps the classes categorical, and the sources' own NoData is carried through so the empty collar stays NoData. Without gdal (`osgeo`) it falls back to core `MosaicToNewRaster`, which writes every block; the tool warns that this is slow, can be very large, and does not guarantee BigTIFF above 4 GB.

`Warp` is used rather than `BuildVRT`: a VRT skips any input whose CRS WKT is not byte identical to the first input's, even when it is the same CRS by definition, and it does so with only a warning. On the per area rasters, whose WKT varies slightly, that silently dropped most areas and truncated the merge. `Warp` tolerates the WKT variants and keeps every area.
...
| `MAJORITY only applies to integer class rasters ... skipped` (Tool 6) | A continuous type (`DEM`, `DSM`, `SLOPE`, `ASPECT`, `SOLARUNI`) was in the selection with `MAJORITY` forced | Expected, the continuous rasters are skipped, not an error. Deselect them; use `MAJORITY` only for the class rasters (`APT`, `APTCLS`, `ASPECT_DIR`, `_RCL`). |
...
| `Inputs were dropped during the merge; the output is incomplete` (Tool 10) | The merged grid came out smaller than the union of the inputs, so the merge lost areas | Should not happen on the `Warp` path; re-run. If it persists, one input has a mismatched CRS or extent; check the reported grid sizes and the offending product. |
| `has NoData ... but the others have ...` / `has no NoData value` (Tool 10) | The product's rasters were written with inconsistent NoData values, so merging them would corrupt the result | Rebuild the class rasters with Tool 9 (which now writes a fixed NoData of 255), then resample (Tool 6) and merge again. Older `APT`/`APTCLS` from before this fix can carry a class-colliding NoData and must be regenerated, not just re-merged. |
